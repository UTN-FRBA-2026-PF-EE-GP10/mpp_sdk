...
| 020 | Firmware+SDK: stream sweep points as they are captured | P2 | M | - (023 wants it) | DONE (on-target confirmed 2026-09-13: points draw one by one in the workbench, and the finished curve matches the bulk read. The bench fixed a request sent too few times, which made the page jump to the full curve; plan file removed) |
...
| 023 | GUI: React + shadcn curve workbench, served from the Pi | P2 | L | 021 (hard), 020 (hard) | DONE (on-target confirmed: curves captured on the board, saved and opened in their panes. Replaced the vanilla-JS page and moved the server to FastAPI, overriding this plan's STOP condition by operator decision. Later work (PRs #93-#96) added the sidebar, run player, demo modes and live runs; plan file removed) |
| 024 | Bench: microcontroller-driven lamp dimmer - design spike | P3 | M | - | TODO |
| 025 | Frontend: CI coverage for `frontend/` (typecheck, lint, build) | P2 | M | - | DONE (plan file removed, see PR history) |
...
| 034 | SDK+scripts: capture and analyze a real closed-loop MPPT run ("runs") | P2 | L | - | DONE (code: `mpp_sdk/runs/`, `run_algorithm.py`/`plot_run.py`, later driven from the web UI. Step 7, the on-target run, moved to plan 040; plan file removed) |
| 035 | Docs: measurement-procedure checklist page (curves, tilt steps, runs) | P2 | M | 034 (hard, Step 4 only) | DONE (plan file removed, see PR history) |
...
| 041 | Docs: one-page guide - the full two-panel setup (A fixed, B tilted) with the workbench | P1 | L (bench + page) | 040 Step 0 before quoting V out | IN PROGRESS (todo list rewritten short 2026-09-21. Session 1, Luxen alone, 2026-09-22 -> page v1; Session 2, panels A and B, later -> page v2) |
| 042 | Workbench: setup mode (single/full) and measurement reports | P1 | L (3 PRs) | - | IN PROGRESS (A: setup toggle; B: reports backend + templates; C: reports frontend) |
