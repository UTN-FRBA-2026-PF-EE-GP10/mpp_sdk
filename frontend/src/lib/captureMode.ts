...
// and a saved curve's provenance can never drift apart:
//
//   'hardware'         Pi connected  - Start Measurement: a real sweep off
//                       a real panel.
//   'firmware-replay'  Demo with Pi  - the "Demo curve" buttons: real SPI,
//                       a curve already stored in the firmware, not
//                       measured this session. Requires a live link to the
//                       Pi - there's nothing to replay from without one.
//   'simulated'        Demo          - no board at all. Bundled fixtures
//                       (lib/demoFixtures.ts), replayed locally
//                       (hooks/useDemoCapture.ts). Called "sandbox" in
//                       code (lib/sandbox.ts) to avoid a third overload of
//                       the word "demo" alongside ConnectionStatus's own
//                       'demo' value (the server's --demo flag, a
//                       simulated sweep source standing in for a board)
//                       and this mode's on-screen name.
//
