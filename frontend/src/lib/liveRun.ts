...
// Only the first three are the safety system actually firing - 'stopped' is
...
const ABORT_REASON_MESSAGES: Record<string, string> = {
  ...
  'output-overvoltage':
    'the safety cutoff fired - converter output voltage exceeded the v_out_max limit. Check the load on the output.',
  'link-down': 'the link to the board was lost mid-run.',
  stopped: 'stopped by the operator.',
}

/** A one-line, human explanation of why a run ended early - the five
