...
During early bench bring-up, at a fixed commanded duty $D = 0.5$ and
...
- **`FirmwareMode::PowerSupply`**: the RP2040 runs as an autonomous
  ...
  $V_\text{out}$ to a fixed setpoint (`POWER_SUPPLY_VOUT_MV`, 5 V)
...
## Why every curve and run records where it came from

A replayed or simulated curve looks exactly like a real measurement once
it is saved as JSON. The same is true for a closed-loop run: a trace from
`SimulatedSource` looks like one from the real SEPIC board. Grading an
algorithm against data that is not what it claims to be would be a
silent, uncorrectable error in the results. So `CurveRecord.source` and
`RunRecord.source` tag where the data came from (`hardware`,
`firmware-replay`, `simulated`, or `unknown` for curves; `hardware`,
`simulated`, or `unknown` for runs). The web server stamps this field
itself; it is never taken from a client request, so a client cannot
mislabel its own data as real.
