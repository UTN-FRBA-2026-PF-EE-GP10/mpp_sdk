...
right of it. $\Delta V \approx 0$ would make $E$ blow up, so the
implementation defines $E=0$ in that case instead of dividing; the rule table
then falls back to the ZE row, so the step depends only on $\Delta E$, not on
holding outright. Each input is scaled and clamped to $[-1,1]$:
...
no output-set integration. This matters for the MCU port.
...
ranges. On the first call there is no previous sample, so the controller
records $(V, P)$ and perturbs by one full $\Delta D_\text{max}$ step (not
graduated), same as P&O's priming step.

## References

Subudhi, B., & Pradhan, R. (2013). *A comparative study on maximum power point tracking techniques for photovoltaic
power systems.* IEEE Transactions on Sustainable Energy.
